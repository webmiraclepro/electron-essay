...
class IndexedDb {
    ...
    public static getInstance(): IndexedDb {
    	if(!this.instance) {
    		this.instance = new IndexedDb(IDB_CONFIGS.dbname);
			this.instance.createObjectStore(IDB_CONFIGS.tableNames);
    	}
    ...
    public async createObjectStore(tableNames: string[]) {
        try {
           ...
           // return false;
           console.log("createObjectStore error")
        }
    }
    ...
    public async deleteStoreData(tableName: string) {
        const tx = this.db.transaction(tableName, 'readwrite');
        const store = tx.objectStore(tableName);
        await store.clear();
        return this.getAllValue(tableName)
