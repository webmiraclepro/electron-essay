...
const ctre = {
	...
	updateElementList: function() {
		...
		if (ctre.hiddenElements.length) {
			...
			for (let elm of ctre.hiddenElements) {
				lines.push(`<tr>
					<td class="ct_selector">${escapeHTML(elm.selector)}</td>
	...
	updateSavedElements: function () {
		//modified by jgr start 2021-11-29
		const data = ctre.hiddenElements.filter(elm => elm.permanent)
		return data
		//modified by jgr end 2021-11-29
	},

	//added by jgr start 2021-11-30
	loadSavedElements: function(hiddenElements, settings) {
		ctre.hiddenElements = hiddenElements;
		ctre.updateCSS();
		ctre.updateElementList();
		ctre.settings = settings;
	},
	//added by jgr end 2021-11-30
	...
	activate: function() {
		...
		div.innerHTML = `
			...
			</span> <span class="version">v1</span>
			<span class="ct_logo small">ESSAY</span>
